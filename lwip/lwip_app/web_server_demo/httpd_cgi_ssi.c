...
extern void RTC_Get_Time(U8_t *hour,U8_t *min,U8_t *sec,U8_t *ampm);	//声明RTC_Get_Timer()函数
extern void RTC_Get_Date(U8_t *year,U8_t *month,U8_t *date,U8_t *week); //声明RTC_Get_Date()函数
...
const char* LEDS_CGI_Handler(int iIndex, int iNumParams, char *pcParam[], char *pcValue[])
{
	U8_t i=0;  //注意根据自己的GET的参数的多少来选择i值范围
//	U8_t beep;
		...
		for (i=0; i<iNumParams; i++) //检查CGI参数: example GET /leds.cgi?led=2&led=4 */
		{
		  if (strcmp(pcParam[i] , "LED1")==0)  //检查参数"led"
		  {
			if(strcmp(pcValue[i], "LED1ON") ==0)  //改变LED1状态
			  DBG_LED_RED_ON; //打开LED1
					else if(strcmp(pcValue[i],"LED1OFF") == 0)
						DBG_LED_RED_OFF; //关闭LED1
		  }
		}
	 }
