...
static err_t low_level_output(struct netif *netif, struct pbuf *p)
{
	...
	struct pbuf *q;
	int l = 0;
	U8_t *buffer=(U8_t *)PDS_soc_get_current_tx_buf_addr(); 
	for(q=p;q!=NULL;q=q->next) 
	{
		memcpy((u8_t*)&buffer[l], q->payload, q->len);
		l=l+q->len;
	} 
	res=PDS_soc_send_ethernet_packet(l);
...
{  
	...
	U8_t *buffer;
	p = NULL;
	frame=PDS_soc_receive_ethernet_packet();
	len=frame.length;//得到包大小
	buffer=(U8_t *)frame.buffer;//得到包数据地址 
	...
	if((ETH->DMASR&ETH_DMASR_RBUS)!=(U32_t)RESET)//当Rx Buffer不可用位(RBUS)被设置的时候,重置它.恢复传输
	{ 
